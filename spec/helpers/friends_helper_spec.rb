...
RSpec.describe FriendsHelper, type: :helper do
  describe 'requests' do
    context 'check request' do
      ...
      let!(:request) do
        Friend.create({ friend1: friend1.id, friend2: friend2.id, relationship: 'request',
                        who_sent_request: friend1.id })
      end
    ...
    context 'make friendship' do
      let!(:current_user) do
        User.create({ nick: 'qwe', email: 'asd@asd', password: 'qwe', password_confirmation: 'qwe' })
      end
      let!(:friend2) { User.create({ nick: 'asd', email: 'qwe@asd', password: 'qwe', password_confirmation: 'qwe' }) }
      let!(:request) do
        Friend.create({ friend1: current_user.id, friend2: friend2.id, relationship: 'request',
                        who_sent_request: current_user.id })
      end
